...
export class GdmLiveAudioVisuals extends LitElement {
  ...
  // FIX: LitElement lifecycle methods should be `protected` to correctly override the base class methods.
  protected firstUpdated() {
    // FIX: Use this.renderRoot instead of this.shadowRoot to access the component's rendered DOM.
    this.canvas = this.renderRoot.querySelector<HTMLCanvasElement>('canvas')!;
    this.canvas.width = 400;
    this.canvas.height = 400;
    this.canvasCtx = this.canvas.getContext('2d');
  }

  // FIX: LitElement lifecycle methods must have a compatible signature with the base class. `private` is too restrictive.
  protected render() {
